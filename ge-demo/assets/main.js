...
function connection(method, url) {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
            changeDatas(JSON.parse(xhr.responseText).posts, method)
        } else {
...
function upVote(postId) {
    document.querySelector('button.arrow.up').addEventListener('click', () => {
        connection('PUT', `${apiUrl}/posts/${postId}/upvote`);
    })
}

function downVote(postId) {
    document.querySelector('button.arrow.down').addEventListener('click', () => {
        connection('PUT', `${apiUrl}/posts/${postId}/downvote`);
    })
}

function deletePost(postId) {
    document.querySelector('li#remove').addEventListener('click', () => {
